...
export const TransferManagementView: React.FC<TransferManagementViewProps> = ({
  ...
  const filteredCases = cases.filter((c) => {
    ...
    if (activeStatusFilter === 'omo_pending' && c.status !== 'omo_pending') return false;
  ...
  const getStatusBadge = (status: TransferStatus, caseType?: TransferCase['caseType']) => {
    switch (status) {
      case 'ai_pending':
        return <span className="px-2.5 py-1 text-xs font-semibold rounded-md bg-[#FAF3E0] text-[#8C6A21] border border-[#EEDB9F]">{caseType === 'web_restock' ? '待採用' : 'AI 建議待採用'}</span>;
      case 'omo_pending':
        return <span className="px-2.5 py-1 text-xs font-semibold rounded-md bg-[#EAF2FB] text-[#315F8B] border border-[#C9DDF1]">待門市接單</span>;
      case 'waiting_source':
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1440px] mx-auto space-y-6 animate-in fade-in-50 duration-200">
      ...
      <div className="bg-white rounded-xl border border-[#E8EAEE] p-4 shadow-[0_2px_8px_rgba(0,0,0,0.02)] flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-1.5 overflow-x-auto pb-1 md:pb-0">
          {[
            { id: 'all', label: '全部案件' },
            { id: 'omo_pending', label: 'OMO 待門市接單' },
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 font-mono font-bold text-[#475569]">
                        <div className="flex items-center space-x-1.5">
                          {c.caseType === 'omo_fulfillment' && (
                            <span className="font-sans text-[10px] bg-[#EAF2FB] text-[#315F8B] border border-[#C9DDF1] px-1.5 py-0.5 rounded">OMO 代出貨</span>
                          )}
                          {c.caseType === 'web_restock' && (
                            <span className="font-sans text-[10px] bg-[#F2EAFB] text-[#6F4A8E] border border-[#DDC9EE] px-1.5 py-0.5 rounded">官網補貨</span>
                          )}
                          {c.isAiGenerated && c.caseType !== 'web_restock' && (
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex items-center space-x-2 text-[#334155]">
                          ...
                          <span className="font-semibold">{c.caseType === 'omo_fulfillment' ? '顧客' : tgt}</span>
                        </div>
                        {c.caseType === 'omo_fulfillment' && <div className="text-[10px] text-[#64748B] mt-1">履約原因：{c.fulfillmentReason}</div>}
                      </td>
                      ...
                      {/* Status */}
                      <td className="py-3.5 px-4">
                        {getStatusBadge(c.status, c.caseType)}
                      </td>
