...
export const TransferDetailModal: React.FC<TransferDetailModalProps> = ({
  ...
  const isOmo = caseItem.caseType === 'omo_fulfillment';
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1 text-xs text-[#374151]">
          ...
          <div className="bg-[#FAFBFD] rounded-xl p-4 border border-[#EDF1F6] flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center space-x-3 w-full sm:w-auto">
              ...
              <div>
                <div className="text-[10px] text-[#9CA3AF] uppercase font-semibold">{isOmo ? '履約門市' : '調出店家'}</div>
            ...
            <div className="flex flex-col items-center justify-center">
              <span className="text-xs font-bold text-[#8C6D3B] bg-[#FAF3E0] px-3 py-0.5 rounded-full border border-[#EEDB9F]">
                {isOmo ? '代出貨' : '調撥'} {caseItem.quantity} 件
              </span>
            ...
            <div className="flex items-center space-x-3 w-full sm:w-auto">
              ...
              <div>
                <div className="text-[10px] text-[#9CA3AF] uppercase font-semibold">{isOmo ? '履約目的地' : '調入店家'}</div>
                <div className="text-sm font-bold text-[#1F2937]">{isOmo ? '顧客' : tgtStore?.name}</div>
                <div className="text-[11px] text-[#6B7280]">{isOmo ? caseItem.deliveryMethod : `店長：${tgtStore?.manager}`}</div>
              </div>
            </div>
          </div>

          {isOmo && (
            <div className="bg-[#F3F7FB] rounded-xl border border-[#D7E4F0] p-4 space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-b border-[#DDE7F0] pb-3">
                <div>
                  <div className="font-bold text-[#25384A]">訂單 {caseItem.orderNumber}</div>
                  <div className="text-[11px] text-[#64748B] mt-1">來源：{caseItem.orderChannel} · {caseItem.deliveryMethod}</div>
                </div>
                <span className="self-start text-[11px] font-bold bg-white text-[#315F8B] border border-[#C9DDF1] px-2.5 py-1 rounded-full">
                  {caseItem.status === 'omo_pending' ? '待門市接單' : '已接單／備貨中'}
                </span>
              </div>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="bg-white rounded-lg p-2 border border-[#E2E8F0]"><div className="text-[10px] text-[#64748B]">訂單前可售</div><div className="font-bold mt-1">1 件</div></div>
                <div className="bg-white rounded-lg p-2 border border-[#E2E8F0]"><div className="text-[10px] text-[#64748B]">本單預留</div><div className="font-bold text-[#315F8B] mt-1">1 件</div></div>
                <div className="bg-white rounded-lg p-2 border border-[#E2E8F0]"><div className="text-[10px] text-[#64748B]">預留後可售</div><div className="font-bold text-[#B45309] mt-1">0 件</div></div>
              </div>
              {caseItem.status === 'omo_pending' && canApproveSource && (
                <button onClick={() => onApproveSource(caseItem.id)} className="w-full bg-[#315F6B] hover:bg-[#274E58] text-white py-2 rounded-lg font-bold text-xs transition-colors">
                  接受代出貨並開始備貨
                </button>
              )}
            </div>
          ...
          {!isOmo && <div className="bg-white rounded-xl border border-[#E5E7EB] p-4 space-y-3">
          ...
          <div className="bg-[#FAFBFD] rounded-xl p-4 border border-[#E9ECF0] space-y-2">
            <div>
              <span className="font-bold text-[#4B5563]">{isOmo ? '履約門市選擇原因：' : '調撥申請原因：'}</span>
              <p className="text-[#1F2937] mt-0.5">{caseItem.transferReason}</p>
            </div>
            {isOmo && caseItem.fulfillmentReason && (
              <div className="pt-2 border-t border-[#EDF1F6]">
                <p className="text-[#315F6B] font-medium leading-relaxed">{caseItem.fulfillmentReason}</p>
              </div>
            )}
          ...
          {isOmo && caseItem.fulfillmentCandidates && (
            <div className="bg-white rounded-xl border border-[#E5E7EB] p-4">
              <div className="font-bold text-[#1F2937] mb-3">候選履約門市比較</div>
              <div className="overflow-x-auto">
                <table className="w-full min-w-[520px] text-left">
                  <thead className="text-[10px] text-[#64748B] bg-[#F8FAFC]"><tr><th className="p-2">門市</th><th className="p-2">可售</th><th className="p-2">預估送達</th><th className="p-2">運費</th><th className="p-2">負荷</th><th className="p-2">結果</th></tr></thead>
                  <tbody className="divide-y divide-[#EEF1F4]">{caseItem.fulfillmentCandidates.map((candidate) => (
                    <tr key={candidate.storeId} className={candidate.selected ? 'bg-[#F1F7F3]' : ''}>
                      <td className="p-2 font-semibold">{stores.find(s => s.id === candidate.storeId)?.name}</td><td className="p-2">{candidate.sellableStock} 件</td><td className="p-2">{candidate.estimatedDelivery}</td><td className="p-2">${candidate.estimatedShippingFee}</td><td className="p-2">{candidate.workload}</td><td className="p-2">{candidate.selected ? <span className="font-bold text-[#39704A]">已選擇</span> : '未選擇'}</td>
                    </tr>
                  ))}</tbody>
                </table>
              </div>
          ...
          {caseItem.status === 'both_confirmed' && (
            <div className="bg-[#EFF6FF] border border-[#BFDBFE] rounded-xl p-4 space-y-3">
              <div className="flex items-center space-x-2 text-[#1E40AF] font-bold">
                ...
                <span>{isOmo ? '門市已接單，請完成備貨並寄送顧客' : '雙方已授權完畢，請進行物流發貨'}</span>
              </div>
              <div className="flex items-center space-x-2">
                <input
                  type="text"
          ...
          {caseItem.status === 'in_transit' && (
            <div className="bg-[#F0FDF4] border border-[#BBF7D0] rounded-xl p-4 flex items-center justify-between">
              ...
              >
                  {isOmo ? '顧客已收件，完成履約' : '門市點收無誤，確認入庫完成'}
              </button>
            </div>
          )}
